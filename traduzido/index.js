	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `[ Já sabem o que acontece com membros movos... kkkkkkk _*${mdata.subject}*_ ] \n*_____________*\n@${num.split('@')[0]}\nSELAMAT BERGABUNG\n *_____________*\nMoga betah Di group!`
				...
				teks = `Adeus troxakkkk @${num.split('@')[0]}👋`
				let buffer = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buffer, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: 'Aguarde!',
				asik: 'Aguarde!',
				success: '✔️ tá ai chefe ✔️',
				error: {
					stick: ' Vish, deu algum problema na conversão da foto para a figurinha ',
					Iv: ' [❗] Link inválido!'
				},
				only: {
					group: '[❗] Esse comando só pode ser usado em grupos! ❌',
					public: '[❗] Este comando está privado, fale com o proprietário do bot para obter acesso! ',
					premium: '[❗] Comando disponível apenas para usuários premium*',
					mod: '[❗] É necessario ser MOD para usar esse comando*',
					benned: 'Você foi banido, então não pode usar comandos',
					ownerG: '[❗]  Recurso especial para o proprietário❌',
					ownerB: '[❗] Recurso especial do proprietário❌',
					userB: `──「 Victor bot」──\nOlá !\nAparentemente você ainda não se cadastrou no bot. Faça isso por meio dos comandos:, \n\nComando : ${prefix}daftar [nome|idade] \nExemplo : ${prefix}daftar Victor|18\n\n──「 Victor bot 」──`,
					admin: '[❗] Esse comando só pode ser usado por admins! ❌',
					Badmin: ' [❗] Esse comando só pode ser usado quando o bot se torna adm! ❌',
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["558187251670@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"994406445742@s.whatsapp.net"]
			const adminbotnumber = ["994406445742@s.whatsapp.net"]
			const frendsowner = ["994406445742@s.whatsapp.net"]
			const premium = ["994406445742@s.whatsapp.net"]
			...
			switch(command) {
				...
				if (!isPublic) return reply(mess.only.public)
					...
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*bruh*', text, { quoted: mek })
					break
					case 'funmenu':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
                    ...
                    if (err) return reply ('Falhou, algo deu errado, tente novamente mais tarde.')
							})
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie a imagem com o adesivo de legenda ${prefix} ou marque/responda a imagem que foi enviada`)
					}
					break
					case 'nobg':
                   ...
                   case 'ytkomen':
                                         if (!isUser) return reply(mess.only.userB)
                                         if (args.length < 1) return reply('cadê o texto, brow')
                                         gh = body.slice(9)
                                         usnm = gh.split("|")[0];
                                         cmn = gh.split("|")[1];
                                         var imgbb = require('imgbb-uploader')
                   ...
                   client.sendMessage(from, modapk(prefix), text, { quoted: mek })
					...
					if (!isOwner) return reply('Você não é proprietário')
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply('Voce não é proprietário')
					...
					if (!isPublic) return reply(mess.only.public)
					if (!isGroup) return reply(mess.only.group)
					if (!isNsfw) return reply('Recurso Nsfw Não Ligado Digite ${prefix}nsfw1, e se quiser desligar digite ${prefix}nsfw 0')
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply('Você não é o proprietário')
			       ...
			       case 'valxlove':
                    ...
                    if (isBanned) return reply(mess.only.benned)    
                    if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, 'Anime harém é bom demais' , text, { quoted: mek })
					break
					case 'delsay':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					...
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o proprietário ou digite *#daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					me = client.user
					uptime = process.uptime()
					client.sendMessage(from,  `*──────────────────*\n*Nama bot:* *${me.name}*\n*─────────────────*\n『 *𝐕𝐈𝐏 𝐔𝐒𝐄𝐑*』\n*──────────────────*\n*•Nome:* *${sender.split("@s.whatsapp.net")[0]}*\n*•Status:* *ACTIVE*\n*──────────────────*\n*Status Bot:* *${kyun(uptime)}*\n*──────────────────*` , text, { quoted: mek, })
					break
					case 'gambar':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o proprietário ou digite *#daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque o indivduo')
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o proprietário ou digite *#daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque o indíduo!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = '╭────「 *PREMIUM👑* 」──*\n│+ *Número* : \n│+ *Status*: *DEACTIVE*\n│ 🙂\n*╰──────「 *Victor bot* 」────'
                ...
                break
                    case 'modbotlist':
					teks = 'Lista dos premium (é grátis, só falar cm o wa.me/994406445742 :\n'
					for (let p of mod) {
						teks += `~> @${p.split('@')[0]}\n`
					}
					...
					client.sendMessage(from, qute, image, { quoted: mek, caption: '*Aqui parcero*' })
					break
					case 'indo1':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o proprietário (é de graça, só chamar no WA.me/994406445742 ou digite*#daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					...
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Chama no WA.me/994406445742 pra ele te disponibilizar o comando ou digite: *#daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
				   ...
				   client.updatePresence(from, Presence.composing) 
					...
					if (!isUser) return reply(mess.only.userB)
					if (isBanned) return reply(mess.only.benned)    
				   if (args.length < 1) return reply('O que está procurando?')
					goo = body.slice(7)
					...
					if (isBanned) return reply(mess.only.benned)    
				   if (!isUser) return reply(mess.only.userB)
				     if (args.length < 1) return reply(`[❗] Envie o comando * ${prefix} calcular [Número] * \nExemplo: ${prefix}calcular 12 * 12 \n * NOTA *: \n- Para uso de multiplicação * \n- Para uso de soma + \n- Para uso de subtração - \n- Para dividir  /`)
				    const Math_js = require('mathjs')
				    mtk = body.slice(6)
				    if (typeof Math_js.evaluate(mtk) !== "number") {
					reply(`"${mtk}", Números não encontrados. [❗] Envie o comando * ${prefix} calcular [Número] * \nExemplo: ${prefix}calcular 12 * 12 \n * NOTA *: \n- Para uso de multiplicação * \n- Para uso de soma + \n- Para uso de subtração - \n- Para dividir  /`)
				} else {
					...
					case 's':
					 if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg.result, size: 'auto', type: 'auto', ranp}).then(res => {
							fs.unlinkSync(media)
							let buffer = Buffer.from(res.base64img, 'base64')
							fs.writeFileSync(ranp, buffer, (err) => {.if (err) return reply ('Falhou, algo deu errado, tente novamente mais tarde')
							})
							exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								fs.unlinkSync(ranp)
								if (err) return reply(mess.error.stick)
		...
		case 'iri':
            ...
            case 'bodoh':
				...
				if (!isOwner) return reply('Você não é o proprietário?')
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						const media = await client.downloadAndSaveMediaMessage(encmedia)
						client.sendMessage(from, 'Bot desliado', text, { quoted: mek })
						ran = getRandom('.webp')
						await ffmpeg(`./${media}`)
							.input(media)
							.on('start', function (cmd) {
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg.result, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							fs.writeFileSync(ranp, buffer444, (err) => {
								if (err) return reply('Falhou, ocorreu um erro, tente novamente mais tarde.')
							})
							exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								fs.unlinkSync(ranp)
								if (err) return reply(mess.error.stick)
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                 ...
                 client.sendMessage(from, 'Essa é minha escrava, minha dona ><',MessageType.text, { quoted: mek} )
            ...
            break
                ...
                if (!isPremium) return reply(mess.only.premium)
                	client.updatePresence(from, Presence.composing) 
					if (!isQuotedVideo) return reply('❌ mande o vídeo ❌')
					reply(mess.wait)
				...
				if (!isUser) return reply(mess.only.userB)
                if (!isPublic) return reply(mess.only.public)
					teks = 'Lista dos números do chat :\n'
					...
					case 'registrar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('você se registrou ')
					if (args.length < 1) return reply(`Parameter Salah\nCommand : ${prefix}registar nome|idade\nExemplo : ${prefix}registrar Victor|18`)
					var reg = body.slice(8)
					var jeneng = reg.split("|")[0];
					var umure = reg.split("|")[1];
						user.push(sender)
						fs.writeFileSync('./src/user.json', JSON.stringify(user))
						client.sendMessage(from, `\`\`\`O registro foi feito com sucesso com: TM08GK8PPHBSJDH10J\`\`\`\n\n\`\`\`Em ${date} ${time}\`\`\`\n\`\`\`[Nome]: ${jeneng}\`\`\`\n\`\`\`[Número]: wa.me/${sender.split("@")[0]}\`\`\`\n\`\`\`[Era]: ${umure}\`\`\`\n\`\`\`Já pode usar o bot\`\`\`\n\`\`\`Por favor\`\`\`\n\`\`\`Use ${prefix}help\`\`\`\n\`\`\`\nTotal de usuários ${user.length}\`\`\``, text, {quoted: mek})
					break
				case 'testime':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
